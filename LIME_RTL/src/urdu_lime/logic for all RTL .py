...
class RTLLimeExplainer:                                                          # Define a class for RTL-aware LIME explanations
    def __init__(self, model, vectorizer, language="Urdu"):                      # Initialize with model, vectorizer, and target language
        ...
        self.explainer = LimeTextExplainer(class_names=self.class_names)         # Initialize the standard LIME text explainer
        # GPT-4o is excellent for all 12 RTL languages
        self.llm = ChatOpenAI(model="gpt-4o", temperature=0.1)                   # Initialize GPT-4o with low temperature for stability
    ...
    def _fix_for_plot(self, text):                                               # Helper to prepare RTL text for Matplotlib rendering
        """Specifically for Matplotlib which cannot render RTL/Joining naturally."""
        try:                                                                     # Attempt to process the text
            reshaped_text = arabic_reshaper.reshape(text)                        # Connect Arabic letters based on position
            return get_display(reshaped_text)                                    # Apply BiDi algorithm to reverse the string for display
    ...
    def run_full_analysis(self, exp, user_tweet, predicted_idx):                 # Execute complete analysis including LLM insights
        ...
        if v_rtl:                                                                # If there are positive features to analyze
            ...
            for f in v_rtl:                                                      # Iterate through positive features
                response = self.llm.invoke(rtl_prompt(f[0], True)).content.strip() # Get explanation from LLM
                print(self._fix_for_terminal(response).rjust(RW))                # Print reshaped LLM response right-aligned
        ...
        if nv_rtl:                                                               # If there are negative features to analyze
            ...
            for f in nv_rtl:                                                     # Iterate through negative features
                response = self.llm.invoke(rtl_prompt(f[0], False)).content.strip() # Get explanation from LLM
                print(self._fix_for_terminal(response).rjust(RW))                # Print reshaped LLM response right-aligned
    ...
    def _run_english_logic(self, exp, exp_list, user_tweet, v_rtl, nv_rtl, target_class, other_class, predicted_idx): # Logic for English translations
        ...
        trans_res = self.llm.invoke(f"Translate this {self.language} text to literal English only for safety analysis, no other sentence: {user_tweet}").content.strip() # Translate text
